...
public class MyBST {
	...
	private boolean removeNode(Node node) {
		...
		//Case 1: Deleting node is leaf node, we can just break the link
		//from node to its parent
		if(node.left == null && node.right== null) { // Leaf Node
			Node parent = parent(node);
			if(parent != null) {
				// Deleting node is a left child of Parent, change parent.left is null
				if(parent.left == node) parent.left = null;
				// Deleting node is a right child of Parent, change parent.right is null
				if(parent.right == node) parent.right = null;
			}
			node = null;
			return true;
		}
		//Case 2: Deleting node has either left or right child
		//  add a link from parent to child of node.
		...
		//Case 3: in this case, find min to the right of node and
		//insert this value into the node; link up the parent of
		//this min with the right child of this min. Replace with predecessor

		if(node.left != null && node.right != null) {
			Node min = findMinNode(node.right);// Successor
	...
	private Node parent(Node t) {// Find parent of t
		return parent(t,root,null); // Initially parent didn't identified so pass null
	}
	...
	private boolean find(Integer x, Node n){
		...
		return (x.compareTo(n.data) < 0) ? // Input is
				find(x, n.left) :
				find(x, n.right);
	}
